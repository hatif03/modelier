...
const VariantCard = ({ variant, onAddToCanvas, onDropIntoPlaceholder }: Props) => {
  ...
  // Read-only when neither canvas action is supplied — used by
  // PreviewOnModelModal, which has no canvas to add results into.
  const hasCanvasActions = Boolean(onAddToCanvas || onDropIntoPlaceholder);
